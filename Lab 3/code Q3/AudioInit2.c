...
extern unsigned char swtheme[];
extern const int SW_SIZE;
...
int probe1()
{
   ...
   /* int           i,j;*/
     ...
     ****************************/
    if(!(pciFindDevice(PCI_VENDOR_ID_CIRRUS,PCI_DEVICE_ID_CRYSTAL_CS4281,0, &pciBusNo, &pciDevNo, &pciFuncNo)==OK)) {
...
void sendToBuffer(void *ptr, int length)
{

   base_addr = malloc(length)
   for(int i =0; i<length;i++)
   {
        *(base_addr+i) = *(ptr+i);  /*The buffer contents are copied*/
   }
   new =1;
}

void loadbuffer(char *b , int size, int pointer,int len)
{
	int i,s;
    int fsize;
	 fsize = len;
        ...
        kk++;
           ...
           /* semTake(SEM_Sample, WAIT_FOREVER);*/
	    	...
	    	if(DTC_DMA_Playback)
	    	{
               ...
               /* memmove(DAC_BUFFER+(DAC_BUFFER_SIZE/2),saved_samples, (DAC_BUFFER_SIZE/2));
               */

	    		loadbuffer( DAC_BUFFER + (DAC_BUFFER_SIZE / 2), DAC_BUFFER_SIZE/2, choice );

	    	}
            else
            {
            	printf("\e[1;40H task1=%d    [%d]  \n", kk );
                /*memmove(DAC_BUFFER,saved_samples, (DAC_BUFFER_SIZE/2));
                */
            	loadbuffer( DAC_BUFFER ,  DAC_BUFFER_SIZE/2,choice);
            }
...
void fun (void)
{
	...
	int abort = 0;
		printf ("\n Please enter the name of the buffer you want to load :");
		printf("\n To load Tune 1 to buffer Press 1:");
		printf("\n To load Tune 2 to buffer Press 2:");
		printf("\n To load Tune 3 to buffer Press 3:\n");
		 do
		{
			choice =getchar();
			/*putchar(c);*/
